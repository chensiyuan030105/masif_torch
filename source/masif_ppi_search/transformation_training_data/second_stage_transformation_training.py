...
def main():
    ...
    training_list  = params["testing_list"]
    ...
    pdb_list = [s.strip() for s in pdb_list] 
    ...
    for target_ix, target_pdb in enumerate(rand_list):
        ...
        #     continue
        print("target_ix =", target_ix, ", target_pdb =", target_pdb)
        ...
        target_pcd = open3d.io.read_point_cloud(target_pc)
        target_mesh = open3d.io.read_triangle_mesh(target_pc)
        ...
        # Assume 9A patches. 
        target_coord = np.load(os.path.join(precomp_dir_9A, target_pdb, 'p1_list_indices.npy'), allow_pickle=True)
        ...
        target_atom_coord_pcd = open3d.geometry.PointCloud()
        target_ca_coord_pcd = open3d.geometry.PointCloud()
        target_atom_coord_pcd.points = open3d.utility.Vector3dVector(np.array(target_atom_coords))
        target_ca_coord_pcd.points = open3d.utility.Vector3dVector(np.array(target_ca_coords))
        ...
        for source_ix, source_pdb in enumerate(rand_list):
            ...
            source_pcd = open3d.io.read_point_cloud(os.path.join(surf_dir, '{}.ply'.format(pdb_id + '_' + chain)))

            source_desc = np.load(os.path.join(desc_dir, source_pdb, 'p2_desc_straight.npy'))
            source_coords = np.load(os.path.join(precomp_dir_9A, source_pdb, 'p2_list_indices.npy'), allow_pickle=True)
            ...
            all_results, all_source_patch, all_source_descs= \
                    multidock(source_pcd, source_coords, source_desc, source_vix, target_patch, target_patch_descs, ransac_iter=ransac_iter) 
            ...
            print("source_pdb =", source_pdb, ", target_pdb =", target_pdb)
            if source_pdb == target_pdb: 
                for j,res in enumerate(all_results):
                    ...
                    print("rmsd =", rmsd, ", fitness =", res.fitness)
