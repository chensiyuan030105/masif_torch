...
def load_config(path: str) -> dict:
    ...
    if exp:
        ...
        if isinstance(cfg.get("site"), dict):
            for k, v in list(cfg["site"].items()):
                if isinstance(v, str):
                    cfg["site"][k] = v.replace("{exp}", str(exp))

    # Minimal required keys for this script
    required = ["ply_file_template", "site"]
    for k in required:
        if k not in cfg or not cfg[k]:
            raise ValueError(f"Config missing required key: {k}")

    if "training_list" not in cfg["site"] or not cfg["site"]["training_list"]:
        raise ValueError("Config missing required key: site.training_list")

    if "masif_precomputation_dir" not in cfg["site"] or not cfg["site"]["masif_precomputation_dir"]:
        raise ValueError("Config missing required key: site.masif_precomputation_dir")
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch precompute MaSIF PPI-search arrays from site.training_list."
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    parser.add_argument(
        "--list",
        default=None,
        help="Optional override for the list file path (otherwise uses site.training_list).",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    list_path = args.list if args.list is not None else cfg["site"]["training_list"]
